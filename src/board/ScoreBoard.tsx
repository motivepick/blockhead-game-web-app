...
const score = (words: string[]) => words.map(it => it.length).reduce((a, b) => a + b, 0)
...
const ScoreBoard = () => {
    ...
    return (
        <table className="min-w-full">
            ...
            <thead className="border-b  bg-indigo-100 border-indigo-200 dark:bg-slate-700 dark:border-slate-600">
                <tr>
            ...
            <tbody>
                {zip(wordsByUser, wordsByComputer).map(([userWord, computerWord], i) => (
                    <tr className="border-b  dark:border-slate-600" key={`move${String(i)}`}>
                        <td className="text-sm text-gray-900 dark:text-gray-200 font-medium px-6 py-4 whitespace-nowrap">
                            {userWord}
                        </td>
                        <td className="text-sm text-gray-900 dark:text-gray-200 font-medium px-6 py-4 whitespace-nowrap">
                            {computerWord}
            ...
            <tfoot>
                <tr>
                    <td className="text-sm text-gray-900 dark:text-gray-200 px-6 py-4 whitespace-nowrap">
                        <b>{t('scoreboardScore')}</b> {score(wordsByUser)}
                    </td>
                    <td className="text-sm text-gray-900 dark:text-gray-200 px-6 py-4 whitespace-nowrap">
                        <b>{t('scoreboardScore')}</b> {score(wordsByComputer)}
                    </td>
                </tr>
            </tfoot>
        </table>
